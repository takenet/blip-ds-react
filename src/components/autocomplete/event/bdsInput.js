...
export default function InputEvent() {
  ...
  useEffect(() => {
    const element = document.getElementById("InputEvent");
    element.addEventListener("bdsInput",(obj) => {
        setResult(true);
        if(obj.detail.data !== null) {
          setInputValue([inputValue, ...obj.detail.data]);
        }
      },[]);
  });
          ...
          >
            <bds-grid direction="column" gap="1">
              ...
              <bds-grid padding="x-3" direction="column" gap="2">
                <bds-autocomplete id="InputEvent" placeholder="Select">
                ...
                {result ? (
                  ...
                  >
                    <bds-grid xxs="4">
                      <bds-paper>
                        <bds-grid padding="2">
                          <bds-typo>Value called by bdsInput: {inputValue}</bds-typo>
                        </bds-grid>
                      </bds-paper>
                    </bds-grid>
                  </motion.div>
