...
export default function CancelEvent() {
  ...
  useEffect(() => {
    const element = document.getElementById("cancelEvent");
    element.addEventListener("bdsCancel",() => {
        setResult(true);
        setTimes(times + 1);
      },[]);
  });
    ...
    >
      <bds-grid direction="column" xxs="12" align-items="center">
        <bds-grid xxs="12" direction="column">
          ...
          <hr />
          <bds-banner context="inside" variant="warning">The event does nothing, will likely be discontinued.</bds-banner>
        </bds-grid>
          ...
          >
            <bds-grid direction="column" gap="1">
              ...
              <bds-grid padding="x-3" direction="column" gap="2">
                <bds-autocomplete id="cancelEvent" placeholder="Select">
                ...
                {result ? (
                  ...
                  >
                    <bds-grid xxs="4">
                      <bds-paper>
                        <bds-grid padding="2">
                          <bds-typo>Times called by bdsCancel: {times}</bds-typo>
                        </bds-grid>
                      </bds-paper>
                    </bds-grid>
                  </motion.div>
