...
export default function BlurEvent() {
  // Put here the event function

  const [resultBlur, setResultBlur] = useState(false);
  const [timesBlur, setTimesBlur] = useState(0);

  useEffect(() => {
    const element = document.getElementById("blurEvent");
    element.addEventListener("bdsBlur",() => {
        setResultBlur(true);
        setTimesBlur(timesBlur + 1);
      },[]);
  });
          ...
          >
            <bds-grid direction="column" gap="1">
              ...
              <bds-grid padding="x-3" direction="column" gap="2">
                <bds-autocomplete id="blurEvent" placeholder="Select">
                ...
                {resultBlur ? (
                  ...
                  >
                    <bds-grid xxs="4">
                      <bds-paper>
                        <bds-grid padding="2">
                          <bds-typo>Times called by bdsBlur: {timesBlur}</bds-typo>
                        </bds-grid>
                      </bds-paper>
                    </bds-grid>
                  </motion.div>
